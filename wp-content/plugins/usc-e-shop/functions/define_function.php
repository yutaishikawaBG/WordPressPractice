	...
	if ( ! function_exists( 'usces_item_uploadcsv' ) ) :
		...
		function usces_item_uploadcsv() {
			...
			if ( isset( $_REQUEST['action'] ) && 'itemcsv' === $_REQUEST['action'] ) {
				...
				if ( $db_check ) {
					$message = __( 'The same product cord is registered.', 'usces' );
					foreach ( $db_check as $d_item ) {
						$message .= ' , ' . $d_item['meta_value'];
					}
				...
				foreach ( $lines as $rows_num => $line ) {
							...
							case USCES_COL_ITEM_CODE:
								if ( 0 === strlen( $data ) ) {
									...
									$query = $wpdb->prepare(
										...
										WHERE meta_key = %s AND meta_value = %s AND post_type = 'post' 
										AND (post_status = 'pending' OR post_status = 'publish' OR post_status = 'draft' OR post_status = 'private' OR post_status = 'future')", 
										'_itemCode',
										$data
									);
									...
									if ( 'upd' === $mode ) {
										...
										$query = $wpdb->prepare(
											"SELECT meta_id, post_id FROM {$wpdb->postmeta} 
											LEFT JOIN {$wpdb->posts} ON ID = post_id
											WHERE post_id <> %d AND meta_key = %s AND meta_value = %s AND post_type = 'post' 
											AND (post_status = 'pending' OR post_status = 'publish' OR post_status = 'draft' OR post_status = 'private' OR post_status = 'future')", 
					...
					if ( ! $check_mode ) {
						...
						if ( $pre_code !== $item_code ) {
							...
							$valstr .= '(' . $post_id . ", '_itemOrderAcceptable','" . $itemOrderAcceptable . "'),";
							...
							// Tag.
							$tags = ( 0 === (int) $usces->options['system']['csv_encode_type'] ) ? explode( ';', trim( mb_convert_encoding( $datas[ USCES_COL_POST_TAG ], 'UTF-8', 'SJIS' ) ) ) : explode( ';', trim( $datas[ USCES_COL_POST_TAG ] ) );

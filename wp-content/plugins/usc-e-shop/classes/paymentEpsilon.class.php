...
jQuery(document).ready( function($) {
	...
	public function settlement_update() {
		...
		$options['acting_settings']['epsilon']['contract_code']  = ( isset( $_POST['contract_code'] ) ) ? trim( wp_unslash( $_POST['contract_code'] ) ) : '';
		$options['acting_settings']['epsilon']['ope']            = ( isset( $_POST['ope'] ) ) ? wp_unslash( $_POST['ope'] ) : '';
		$options['acting_settings']['epsilon']['card_activate']  = ( isset( $_POST['card_activate'] ) ) ? wp_unslash( $_POST['card_activate'] ) : '';
		$options['acting_settings']['epsilon']['multi_currency'] = ( isset( $_POST['multi_currency'] ) ) ? wp_unslash( $_POST['multi_currency'] ) : '';
		$options['acting_settings']['epsilon']['3dsecure']       = ( isset( $_POST['3dsecure'] ) ) ? wp_unslash( $_POST['3dsecure'] ) : '';
		$options['acting_settings']['epsilon']['process_code']   = ( isset( $_POST['process_code'] ) ) ? wp_unslash( $_POST['process_code'] ) : '';
		$options['acting_settings']['epsilon']['conv_activate']  = ( isset( $_POST['conv_activate'] ) ) ? wp_unslash( $_POST['conv_activate'] ) : '';
		$options['acting_settings']['epsilon']['conv_limit']     = ( isset( $_POST['conv_limit'] ) ) ? wp_unslash( $_POST['conv_limit'] ) : '';
	...
	<div class="settlement_service"><span class="service_title"><?php echo $this->acting_formal_name; ?></span></div>
			<?php
			if ( isset( $_POST['acting'] ) && 'epsilon' == $_POST['acting'] ) :
				if ( '' != $this->error_mes ) :
					?>
	...
	public function acting_transaction() {
		...
		if ( ! isset( $_GET['acting_return'] ) && isset( $_GET['trans_code'] ) && isset( $_GET['user_id'] ) && isset( $_GET['result'] ) && isset( $_GET['order_number'] ) ) {
			wp_redirect(
				add_query_arg(
					array(
						'acting'        => 'epsilon',
						'acting_return' => '1',
						'trans_code'    => wp_unslash( $_GET['trans_code'] ),
						'user_id'       => wp_unslash( $_GET['user_id'] ),
						'result'        => wp_unslash( $_GET['result'] ),
						'order_number'  => wp_unslash( $_GET['order_number'] ),
					),
					USCES_CART_URL
				)
			);
			...
			foreach ( $_POST as $key => $value ) {
				$post_data[ $key ] = mb_convert_encoding( $value, 'UTF-8', 'SJIS' );
			}
			...
			if ( '1' == $post_data['paid'] ) {
				...
				$mquery = $wpdb->prepare( "UPDATE {$wpdb->prefix}usces_order_meta SET `meta_value` = %s WHERE `meta_key` = %s AND `order_id` = %d", serialize( $post_data ), 'settlement_id', $order_id );
	...
	public function register_order_data( $args ) {
		...
		if ( isset( $_REQUEST['acting'] ) && isset( $_REQUEST['acting_return'] ) && isset( $_REQUEST['trans_code'] ) && 'epsilon' == wp_unslash( $_REQUEST['acting'] ) ) {
			$usces->set_order_meta_value( 'settlement_id', wp_unslash( $_GET['order_number'] ), $order_id );
			$usces->set_order_meta_value( 'wc_trans_id', wp_unslash( $_GET['order_number'] ), $order_id );
		}
	}

...
class usces_settlement {
	...
	public function __construct() {
		 ...
		 *);
		 */
		if ( in_array( 'veritrans', $this->available_settlement ) ) {
			unset( $this->available_settlement['veritrans'] );
		}
		if ( ! in_array( 'epsilon', $this->available_settlement ) ) {
			$this->available_settlement['epsilon'] = 'イプシロン';
		}
		if ( ! in_array( 'paypal_cp', $this->available_settlement ) ) {
			$this->available_settlement['paypal_cp'] = __( 'PayPal Commerce Platform', 'usces' );
		}
		if ( ! in_array( 'sbps', $this->available_settlement ) ) {
			$this->available_settlement['sbps'] = 'SBペイメントサービス';
		}
		if ( ! in_array( 'dsk', $this->available_settlement ) ) {
			$this->available_settlement['dsk'] = 'DSK 電算システム';
		}
		if ( ! in_array( 'escott', $this->available_settlement ) ) {
			$this->available_settlement['escott'] = 'e-SCOTT Smart';
		}
		if ( ! in_array( 'welcart', $this->available_settlement ) ) {

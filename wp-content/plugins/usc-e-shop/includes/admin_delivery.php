...
var cool_categories = <?php echo json_encode( array(
	...
	operation = {
		...
		add_delivery_method : function() {
			...
			s.data = "action=shop_options_ajax&mode=add_delivery_method&name=" + name + "&time=" + time + "&charge=" + charge + "&days=" + days + "&nocod=" + nocod + "&intl=" + intl + "&cool_category=" + cool_cat;
		...
		update_delivery_method : function() {
			...
			var s = operation.settings;
			s.data = "action=shop_options_ajax&mode=update_delivery_method&name=" + name + "&id=" + id + "&time=" + time + "&charge=" + charge + "&days=" + days + "&nocod=" + nocod + "&intl=" + intl + "&cool_category=" + cool_cat;
			$.ajax( s ).done(function( response ){
				var data = JSON.parse(response);
				var id = data.id - 0;
				for(var i = 0; i < delivery_method.length; i++){
		...
		make_delivery_cool_category : function(selected) {
			...
			for(var i=0; i<cool_categories.length; i++){
				if(selected === cool_categories[i]['id']){
					option += '<option value="'+cool_categories[i]['id']+'" selected="selected">'+cool_categories[i]['name']+'</option>'+"\n";
				}else{
					option += '<option value="'+cool_categories[i]['id']+'">'+cool_categories[i]['name']+'</option>'+"\n";
				}
			}
			option += '</select>';

...
function wel_get_extra_data( $the_item = false, $cache = true ) {
	...
	$post_id = $WelItem->get_id();
	$extra   = apply_filters( 'usces_filter_get_post_user_custom', $extra, $extra, $post_id );
	return $extra;
}
...
function wel_get_main_pict_id_by_code( $item_code, $cache = true ) {
	...
	$WelItem = new Welcart\ItemData( null, false );
	$post_id = $WelItem->get_id_by_item_code( $item_code, $cache );
	$WelItem->set_data( $post_id, $cache );
	$product = $WelItem->get_product();

	if ( NEW_PRODUCT_IMAGE_REGISTER::$opts['switch_flag'] && isset( $product['itemPicts'] ) ) {
